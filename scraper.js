...
const scholarResultsCallback = (resolve, reject) => (error, response, body) => {
  if (error) {
    ...
    if (
      ...
      reject(
        new Error(
          'You have made too many requests too quickly and are being rate-limited by Google Scholar. See: https://support.google.com/websearch/answer/86640.'
        )
      )
    } else {
      reject(
    ...
    if (!results.length)
      return reject(new Error('your Google Scholar query returned no results'))
